    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-white">Service Catalog</h1>
            <p className="mt-1 text-sm text-zinc-400">Manage services and pricing for quotes and invoices.</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
            <h3 className="mb-4 text-sm font-semibold text-white">{editing ? "Edit Service" : "New Service"}</h3>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-zinc-400">Unit Price ($)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-zinc-400">Unit Type</label>
                <input
                  value={form.unitType || ""}
            ...
            <div className="mt-4 flex gap-2">
              <button onClick={handleSave} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500 transition-colors">
                {editing ? "Update" : "Create"}
              </button>
              <button onClick={() => setShowForm(false)} className="rounded-lg border border-zinc-700 px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-800 transition-colors">
                Cancel
              </button>
            </div>
          </div>
        ...
        {/* Content */}
        {loading ? (
          <div className="py-20 text-center text-sm text-zinc-500">Loading catalog...</div>
        ) : grouped.length === 0 ? (
          <div className="py-20 text-center text-sm text-zinc-500">No services found.</div>
        ) : (
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {categoryItems.map((item) => (
                    <div
                      key={item.id}
                      className={cn(
                        "group relative rounded-xl border bg-zinc-900/40 p-4 transition-all hover:border-emerald-500/30 hover:bg-zinc-900/60",
                    ...
                    >
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="flex items-center gap-1.5 text-sm font-semibold text-emerald-400">
                          ...
                          <span className="text-xs font-normal text-zinc-500">/{item.unitType}</span>
                        </div>
                        <span className={cn("text-[10px] font-medium uppercase tracking-wider", item.taxable ? "text-amber-500" : "text-zinc-600")}>
